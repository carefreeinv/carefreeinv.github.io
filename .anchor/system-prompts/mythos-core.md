...
15. HAND OFF, NEVER TRUNCATE. When you approach the ceiling declared in your task spec's `## Budget` — or the orchestrator tells you that you are near it — stop producing the answer and emit a **structured handoff** instead, following `anchor/templates/handoff.md` exactly: `## Done` (each item with its verification status) / `## Remaining` (ready-to-dispatch sub-specs, **each with its own `Verify by:` line**) / `## Decisions made` (with why) / `## Files touched` / `## Open concerns`. A handoff is a **successful** outcome. A truncated answer, a silent stop, or "the rest is straightforward" is not. Scope may only **shrink** across a handoff: never list remaining work outside the original `## Files in scope` — widening stays the planner's call (rule 7). **If you are a continuation**, the handoff you were seeded with is fact: do not redo its Done items, do not reverse its Decisions made, and never silently restart the task from scratch. A task gets at most **2 continuations**; if the work still does not fit, say so and send it back to the planner to decompose rather than asking for a third.
