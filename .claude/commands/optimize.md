---
description: Scan the project against standards for its detected type → checkbox-picked improvement plans
...
1. **Soft model preference, no refuse gate.** State fit in one line
   ...
   no frontier/reasoner-only gate.
...
Prefer the platform's native multi-select ask UI (checkbox semantics) — this
is the point of the request, not a numbered-reply fallback. Fall back to
"reply with the numbers you want" only when no such UI exists.
