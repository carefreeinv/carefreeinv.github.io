...
name: audit
description: >
  Exhaustive security audit of the current codebase (including dependencies) and
  create prioritized bug-fix plans under .plans so teams can close security holes.
  Frontier/reasoner only by default. Use when the user runs /audit, asks for a
  security audit, dependency audit, CVE scan with plans, or sec-audit.
argument-hint: "[path|--dry-run|--write|--to drafts|bugs|--force-model|--deps-only|--code-only|--include-noise|--continue]"
disable-model-invocation: false
metadata:
  short-description: "Security audit → prioritized bug plans"
---
...
(`spawn_subagent`, read-only / capability_mode read-only when available) so the
orchestrator is not the sole author of severity — same spirit as `/review` AI pass.
...
Prefer `ask_user_question` when available:
