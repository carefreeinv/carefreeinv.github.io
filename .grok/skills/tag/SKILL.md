...
name: tag
description: >
  Detect this repo's version-tag scheme and show/suggest/create the next
  local annotated git tag via /tag. Never pushes tags or branches. Use when
  the user runs /tag, asks what version to cut next, or wants a version tag
  created at the current or a named commit.
argument-hint: "[--status|--suggest [patch|minor|major]|vX.Y.Z|--create vX.Y.Z] [--at <ref>] [--allow-dirty]"
disable-model-invocation: false
metadata:
  short-description: "Local annotated version tags — detect, suggest, create"
---
